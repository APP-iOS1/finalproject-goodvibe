...
struct MapUIView: UIViewRepresentable {
  ...
  func makeUIView(context: Context) -> MKMapView {
    ...
    trackingButton.frame = CGRect(origin: CGPoint(x: 343, y: 470), size: CGSize(width: 40, height: 40))
    trackingButton.layer.cornerRadius = 20
    //    traclomgButton.frame =
    
    
    //    let originPoint: CGPoint = CGPoint(x: maps.frame.width-70, y: maps.frame.height-85)
